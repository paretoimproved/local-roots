...
import { CreateCSAShareInput, createShare } from "@/api/shares.api";
...
export default function NewShare() {
  ...
  const [formData, setFormData] = useState<CreateCSAShareInput>({
    name: "",
    description: "",
    price: 0,
    frequency: "weekly",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Failed to create the share. Please try again.");
    ...
    return (
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="text-3xl font-bold tracking-tight">Create CSA Share</h1>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <div className="flex items-center space-x-2">
            <input 
              ...
              className="h-4 w-4"
            />
            <label htmlFor="available" className="text-sm font-medium">Make this share available immediately</label>
          </div>
        </div>
